//  g++ -g complex3.cpp -std=c++11 -o complex3
// Radian Version
...
int main ()    {
	...
	cout << setw(53) << "Version: R A D I A N Measure\n" << endl;
...
std::string cis(complex<double> z)  {
	...
	double theta = arg(z);
	if (theta < 0) theta += 2.0*PI;   // see above note
	...
	case 2:
		{
			...
			cin >> rho;
			cout << "\nEnter direction in radians (phase angle THETA): ";
			cin >> theta;
			if (std::fabs(theta) < tol)  theta = 0; // if theta is zero
			complex<double> C(std::polar(rho, theta));
			return C;
		}
	}
...
int angle_theta(complex<double> z)   {
	double theta = arg(z);
	if (theta < 0) theta += 2.0*PI;   // see note above cis(z) definition
	else if (std::fabs(theta) < tol)  theta = 0; // if theta is zero
	cout << "phase angle in radians on interval (0, 2*PI]: " << theta;
	return(0);
}
...
void Roots_of_Unity(int Nth)    {
    ...
    for (int k = 0; k < Nth; ++k)  {
	  cout << "root " << j << ": ";
	  //K = 2.0*PI*k/(Nth);
      cout << cis(std::polar(1.0, 2.0*k/(Nth))) << " = "
	            << xyi(std::polar(1.0, 2.0*k/(Nth)));
	  cout << endl;
	  j++;
	  }
}
...
int roots(complex<double> z, int n)   {
       ...
       if ( (std::fabs(z.real() - 1) <=  tol) && (z.imag() <= tol) ) Roots_of_Unity(n);
		   ...
		   cout << " So, the reference angle = arccos(" << std::fabs(z.real()/abs(z))
		        << ") = " << acos(std::fabs(z.real()/abs(z)));

		   cout << "\n The radian measure of angle t = ";
		   ...
		   case 2:
			   cout << "PI - arccos(" << std::fabs(z.real()/abs(z)) << ") = ";
			   phi = PI - angle;
			   break;
		   ...
		   case 4:
			   cout << "2*PI - arccos(" << std::fabs(z.real()/abs(z)) << ") = ";
			   phi = 2.0*PI - angle;
			   break;
	      ...
	      pause();
		  cout << "\n Thus " << n << "*t differs from " << phi
			   << " by an integral multiple of 2*PI ~ 6.2831853... :\n\n";

		   cout << n << "*t = " << phi << " + k*2*PI, k any integer\n"
		        << "  t = " << phi/n << " + k*" << 2.0*PI/n
				<< ", k any integer.\n\n Letting k = ";
		   for (int i = 0; i < n; ++i) cout << i << ", ";
		   cout << "we have t = ";
		    for (int i = 0; i < n; ++i) {
				cout << phi/n + i*2.0*PI/n;
				if ( i != (n - 1) ) cout << ", ";
			}
